from checkers.checker import Checker, CheckerType
...
class Field:
    ...
    def white_checkers_count(self) -> int:
        ...
        for checkers in self.__checkers:
            count += sum(checker.type in WHITE_CHECKERS for checker in checkers)
        return count
    ...
    def black_checkers_count(self) -> int:
        ...
        count = 0
        for checkers in self.__checkers:
            count += sum(checker.type in BLACK_CHECKERS for checker in checkers)
        return count
    ...
    def white_score(self) -> int:
        ...
        for checkers in self.__checkers:
            score += sum(
                (checker.type == CheckerType.WHITE_REGULAR) + (checker.type == CheckerType.WHITE_QUEEN) * 3 for checker
                in checkers)
        return score
    ...
    def black_score(self) -> int:
        ...
        for checkers in self.__checkers:
            score += sum(
                (checker.type == CheckerType.BLACK_REGULAR) + (checker.type == CheckerType.BLACK_QUEEN) * 3 for checker
                in checkers)
        return score