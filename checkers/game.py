...
from checkers.checker import CheckerType, SideType
...
class Game:
    ...
    def _draw_field_grid(self):
        ...
        for y in range(self._field.y_size):
            for x in range(self._field.x_size):
                ...
                # Отрисовка рамок у необхдимых клетоко
    ...
    def mouse_down(self, event: Event):
        ...
        if not self._field.is_within(x, y):
            return
    ...
    def __handle_player_turn(self, move: Move):
        ...
        required_moves_list = list(filter(lambda required_move: move.to_x == required_move.from_x and move.to_y == required_move.from_y, self.__get_required_moves_list(player_side))) #Запросить лист доступных ходов
    ...
    def __predict_optimal_moves(self, side: SideType) -> list[Move]:
        ...
        if optimal_moves:
            ...
            for move in choice(optimal_moves):
                if side == SideType.WHITE and self._field.type_at(move.from_x, move.from_y) in BLACK_CHECKERS:
                    break
                elif side == SideType.BLACK and self._field.type_at(move.from_x, move.from_y) in WHITE_CHECKERS:
                    break
            ...
            for x in range(self._field.x_size):
                ...
                if self._field.type_at(x, y) == friendly_checkers[0]:
                    for offset in MOVE_OFFSETS:
                        if not (self._field.is_within(x + offset.x * 2, y + offset.y * 2)):
                            continue
                ...
                # Для дамки
                    ...
                    for offset in MOVE_OFFSETS:
                        ...
                        for shift in range(1, self._field.size):
                            if not (self._field.is_within(x + offset.x * shift, y + offset.y * shift)):
                                continue
